...
public class ListAllCarNamesCommand implements CommandWithResult<Map<Integer, String>> {
    ...
    public void execute() {
        try {
            ...
            stmt = connection.prepareStatement("SELECT id,nev FROM auto;");
            ...
            rs.close();
            connection.close();
            log.info(String.format("Sikeres listázás. Autók száma:%d", result.size()));
...
//        CommandWithResult<Map<Integer, String>> command = new ListAllCarNamesCommand();
//        command.execute();
